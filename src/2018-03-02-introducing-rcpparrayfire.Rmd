...
of a European put option that was recently handled with [R, Rcpp and RcppArmadillo](http://gallery.rcpp.org/articles/black-scholes-three-ways/). Porting
this code to RcppArrayFire is straight forward:
...
How does ArrayFire fare in this respect? Even without a high performance GPU 
the performance boost from using ArrayFire can be quite impressive. Using the same
...
#> -1- POCL: pthread-Intel(R) Core(TM) i7-6600U CPU @ 2.60GHz, 17510 MB
...
#> 4   AF          100   0.846    1.000
#> 2 Arma          100   5.650    6.678
#> 3 Rcpp          100  10.440   12.340
#> 1    R          100  15.494   18.314
```

Here `RcppArrayFire::arrayfire_info()` tells us about the installed GPU and CPU,
since OpenCL drivers for both GPU and CPU are installed.

However, the results change dramatically, if fewer options are evaluated:
...
#>   test replications elapsed relative
#> 2 Arma         1000   0.017    1.000
#> 3 Rcpp         1000   0.021    1.235
#> 1    R         1000   0.033    1.941
#> 4   AF         1000   0.207   12.176
```
