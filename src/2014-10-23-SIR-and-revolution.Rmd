...
library("plyr")
MC1 <- 20000
dfPriors <- ddply(dfPriorInfo,
                  "id",
                  .fun = (function(X) data.frame(draws = (do.call(paste("r",
                                                                        X$dist,
                                                                        sep = ""
                                                                        ),
                                                                  list(MC1,
...
NumericVector samplePost (const NumericVector prdraws,
    ...
    NumericVector podraws = RcppArmadillo::sample(prdraws,
                                                  D,
                                                  true,
                                                  wts
                                                  ) ;
...
f1 <- function(X) {
    ...
    return(data.frame(theta1 = t1,
                      theta2 = t2
                      )
           )
}

dfPost <- ddply(dfPriors,
                "id",
                f1
                )
...
dfSumm <- ddply(dfPost,
                "id",
                function (X) with(X, mean(theta1 - theta2 > 0))
                )

dfPriorIll <- ddply(dfPriorInfo,
                    "id",
...
ggplot(dfPost) +
    geom_histogram(aes(x = theta1 - theta2,
                       y = ..density..
                       ),
                   alpha = .5,
                   fill = muted("lightblue")
                   ) +
    geom_vline(aes(x = 0),
               size = 2,
               color = "white"
    ...
    facet_grid(id ~ .,
               labeller = (function(X, Y) paste(dfPriorInfo$dist[Y],
                                                " (",
                                                dfPriorInfo$par1[Y],
                                                ",",
                                                dfPriorInfo$par2[Y],
