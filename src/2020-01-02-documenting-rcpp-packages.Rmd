...
The first command compiles the `.cpp` code and generates `R/RcppExports.R`. You will notice that the roxygen comment blocks are faithfully transcribed. The second command then generates the `man/*.Rd` files based on these roxygen blocks. The last command simply installs the new package, making it available through the standard `library(testpkg)` interface.
