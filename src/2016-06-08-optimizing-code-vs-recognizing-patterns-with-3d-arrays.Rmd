...
```{r setup, echo = F}
# Needed a higher width for benchmark output to be on the same line.
options(width = 90)
```
...
```{r vec_creation, eval = F, engine = 'Rcpp'}
#include <RcppArmadillo.h>
...
```{r matrix_creation, eval = F, engine = 'Rcpp'}
...
```{r matrix_calculation, eval = F, engine = 'Rcpp'}
...
```{r dim_to_cube, eval = F, engine = 'Rcpp'}
